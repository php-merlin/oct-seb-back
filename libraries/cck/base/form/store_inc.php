...
if ( count( $fields ) ) {
	...
	foreach ( $fields as $field ) {
		...
		if ( !$config['copyfrom_id'] ) {
			if ( !$id && $field->live && ( ( $field->variation == 'hidden' || $field->variation == 'hidden_anonymous' || $field->variation == 'disabled' || $field->variation == 'value' ) || ( ( $field->variation == 'hidden_auto' || $field->variation == 'hidden_isfilled' || $field->variation == 'disabled_isfilled' ) && $session->has( 'cck_hash_live_'.$field->name ) ) ) ) {
				$toBeChecked	=	true;
			}
		}
